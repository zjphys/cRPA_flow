...
child non-login shell. A failed setup, simple command or pipeline stops the job;
...
### Submission retries and environment setup

Put runtime environment loading in `EXECUTION_SETUP`, for example:

```bash
EXECUTION_SETUP='source /path/to/vasp_env.sh'
STAGE_COMMANDS[default]='mpirun -np "$SLURM_NTASKS" vasp_std'
```

Commands inherit the environment without reading login profiles again. Legacy
setup in `SBATCH_TEMPLATE` still works with failure checking, provided every
`#SBATCH` directive precedes executable shell code.

`submit`, `submit-wannier` and `submit-crpa` record accepted jobs under
`.workflow-submissions`. Repeating the same command checks `squeue`/`sacct`,
reuses active or successfully completed jobs with unchanged scripts/dependencies,
and submits missing stages. Changing `--job-name` does not rename reused jobs.
Completed predecessors need no new scheduler dependency, even if their IDs have
aged out of the live controller. To deliberately start a new run:

```bash
crpa-workflow submit --resubmit --job-name Pu
# Or: crpa-workflow submit-wannier --resubmit
# Or: crpa-workflow submit-crpa --resubmit
```

`--resubmit` first requires every tracked job in the case to be terminal, then
archives selected records. If that submission stops partway through, retry
**without** `--resubmit` to continue the new run. Failed parents may leave children
pending in Slurm; inspect and cancel those obsolete children before resubmitting.
No jobs are automatically cancelled. Unknown states, unavailable accounting, or
an interrupted/ambiguous submission (`*.pending`) block new submissions. Inspect
Slurm before reconciling records; removing records blindly can create duplicates.
Executable overrides are `SUBMIT_COMMAND`, `SQUEUE_COMMAND`, and `SACCT_COMMAND`.
`doctor submit` also checks `flock` and `sha256sum`.

Tracking only covers these workflow commands after this update. Jobs submitted
with direct `sbatch`, or by earlier releases, must be checked separately. The
lock requires filesystem support for `flock` on the calculation directory.
`status` remains a view of output files, not a scheduler-status command.

Batch operations now record `preparing`, `prepared`, `submitted`, or `ran` in
`.batch-workflow-state`; `submitted` means accepted by Slurm, not finished.
Only completed operations of the requested kind are skipped successfully.
Incomplete/legacy cases return a failure: inspect and resume `submit`/`run` from
the individual case. Use batch `--force` to recover preparation only when there
are no submission records; tracked cases cannot be regenerated by batch force.

Existing generated `job.sh` files do not update with the package. After old jobs
end, regenerate the appropriate stage to obtain the shell fixes, reviewing
`--force` because it can overwrite scientific inputs/results. For a minimal
manual update, replace `exec bash -lc` with `exec bash -c`, and put
`set -euo pipefail` after the final Slurm directive but before any `source` command.
